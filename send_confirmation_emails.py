...
def send_confirmation_email(registration):
    """Send confirmation email to a single registration"""
    headers = {
...
def main():
    ...
    for idx, registration in enumerate(registrations, 1):
        print(f"[{idx}/{total}] Sending to {registration.email}...")
        success, result = send_confirmation_email(registration)
        ...
        if success:
            ...
            failed += 1
            failed_emails.append(registration.email)
